...
<div>
    ...
    <div class="container">
      ...
      <el-table
        ...
        fit
      >
        <el-table-column label="所属销售区域" min-width="140px">
        ...
        <el-table-column label="仓库地址" min-width="140px">
          <template slot-scope="scope">
            <span>{{ scope.row.address}}</span>
          </template>
        </el-table-column>
        <el-table-column label="管理员" min-width="140px">
          <template slot-scope="scope">
        ...
        <el-table-column label="仓库状态" min-width="140px">
          <template slot-scope="scope">
            <el-button @click="changeStatus(scope.row)" type="danger" v-if="scope.row.status==0">禁用</el-button>
            <el-button @click="changeStatus(scope.row)" type="primary" v-if="scope.row.status==1">启用</el-button>
          </template>
        </el-table-column>
        <el-table-column label="仓库使用情况" min-width="140px">
          <template slot-scope="scope">
            <el-progress :text-inside="true" :stroke-width="26" :percentage="scope.row.stock/scope.row.max*100 | rounding"></el-progress>
          </template>
        </el-table-column>
        <el-table-column label="仓库最大容量(件)" min-width="140px">
          <template slot-scope="scope">
        ...
        <el-table-column fixed="right" label="操作" width="150">
         ...
         <!-- <el-table-column fixed="right" label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.status==1" @click="handleUpdate(scope.row)"  type="primary" >入库</el-button>
            <el-button v-if="scope.row.status==2" @click="handleDelete(scope.row.id)"  type="danger" >出库</el-button>
          </template>
        </el-table-column> -->
      </el-table>
    </div>
...
import {search,highSearch,deleteStoreHouse} from '@/api/storeHouse/index';
...
export default {
    ...
    data(){
        return{
            list:null,
            subOptionList:[
              {id:'area',name:'所属销售区域',value:'',type:'normal'},
              {id:'manager',name:'管理员',value:'',type:'normal'},
              {id:'address',name:'仓库地址',value:'',type:'normal'},
              {id:'phone',name:'联系电话',value:'',type:'normal'},
              {id:'name',name:'仓库名称',value:'',type:'normal'},
    ...
    methods:{
        ...
        updateStoreHouse(data).then(res=>{
            this.commonSearch()
        })
        },
        handleDelete(data){
            this.listLoading=true
           deleteStoreHouse(data).then(res=>{
